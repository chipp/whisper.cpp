...
if [ "$mname" = "-h5" ]; then
  ...
  mpath="$3"
  echo "$mpath"
  python3.11 models/convert-h5-to-coreml.py --model-name "$mname" --quantize --model-path "$mpath" --encoder-only True
else
  python3.11 models/convert-whisper-to-coreml.py --model "$mname" --encoder-only True --optimize-ane True
fi
